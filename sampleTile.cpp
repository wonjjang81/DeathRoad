...
HRESULT sampleTile::init(string imgName, ATTRIBUTE att, TILE_TYPE tileType, TYPE_ATTRIBUTE typeAttr, ANCHOR_TYPE anchorType, OVERLAPPOSITION overPosition, float scale)
{
	//샘플타일 정보
	_sTile.imgName = imgName;
	_sTile.att = att;
	_sTile.type = tileType;
	_sTile.typeAtt = typeAttr;
	_sTile.anchor = anchorType;
	_sTile.overPos = overPosition;
	_sTile.scale = scale;
	_sTile.showX = 740 + 240;
	_sTile.showY = 100 + 450;
	_sTile.keyMoveX = 0;
	_sTile.keyMoveY = 0;
	...
	_tileSample->tileSetup(_sTile.imgName, 740, 100, _sTile.att, _sTile.type, _sTile.typeAtt, _sTile.anchor, _sTile.overPos, _sTile.scale);
