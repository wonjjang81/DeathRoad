...
BOOL CALLBACK doorIDProc(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	int doorID = 0;
	mapTool* pThis = (mapTool*)GetWindowLongPtr(hDlg, GWLP_USERDATA);
	...
	switch (iMessage)
	{
		...
		case WM_COMMAND:
			...
			{
				case IDOK:
					doorID = GetDlgItemInt(hDlg, IDC_EDIT1, NULL, FALSE);
					pThis->setDoorID(doorID);

				case IDCANCEL:
					EndDialog(pThis->getHandleDoorID(), 0);
				return TRUE;
			}
